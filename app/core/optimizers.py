...
def optimize(
                    ...
                    for i in range(i_start, i_end):
                        for j in range(j_start, j_end):
                            ...
                            if fac > 0:
                                ...
                                sH.append(fac)
    H = coo_matrix(
        (np.array(sH), (np.array(iH), np.array(jH))), shape=(nel, nel)
    ).tocsc()
    Hs = H.sum(1)
    ...
    for i in active_supports_indices:
        node_idx = (
            (sz[i] * (nelx + 1) * (nely + 1) if is_3d else 0)
            + sx[i] * (nely + 1)
            + sy[i]
        )
        if "X" in sdim[i]:
    ...
    for i in active_iforces_indices:
        node_idx = (
            (fiz[i] * (nelx + 1) * (nely + 1) if is_3d else 0)
            + fix[i] * (nely + 1)
            + fiy[i]
        )
    ...
    for i in active_oforces_indices:
        node_idx = (
            (foz[i] * (nelx + 1) * (nely + 1) if is_3d else 0)
            + fox[i] * (nely + 1)
            + foy[i]
        )
        d_val = 4 / 100.0  # Arfirtificial spring stiffness
    ...
    if is_3d:
        fiz_active = np.array(fiz)[active_iforces_indices]
    fox_active = np.array(fox)[active_oforces_indices]
    foy_active = np.array(foy)[active_oforces_indices]
    if is_3d:
        foz_active = np.array(foz)[active_oforces_indices]
    ...
    x = initializers.initialize_material(
        init_type, volfrac, nelx, nely, nelz, all_x, all_y, all_z
    )
    xold = x.copy()
    xPhys = x.copy()
    ...
    while change > 0.01 and loop < n_it:
        ...
        for i in active_regions_indices:
            ...
            if rshape[i] == "□":  # Cube/Square
                for ez in range(
                    max(0, int(rz[i] - r)), min(nelz, int(rz[i] + r)) if is_3d else 1
                ):
                    for ex in range(max(0, int(rx[i] - r)), min(nelx, int(rx[i] + r))):
                        for ey in range(
                            max(0, int(ry[i] - r)), min(nely, int(ry[i] + r))
                        ):
                            if rshape[i] == "□":  # Cube/Square
                                idx = (
                                    (ez * nelx * nely if is_3d else 0) + ex * nely + ey
        ...
        if filter_type == "Sensitivity":
            if is_3d:
                dc[:] = np.asarray((H * (x * dc))[np.newaxis].T / Hs)[
                    :, 0
                ] / np.maximum(0.001, x)
            else:
